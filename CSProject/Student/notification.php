<?php
// ------------------------------------------------------------------
// Student-side notification bell dropdown. Include this once, anywhere
// in the page (placement in the DOM doesn't matter — the panel is
// positioned with CSS), on any Student page that already has the
// #ttq-notif-toggle button in its header (see header.php).
//
// Pulls whatever the admin broadcast to 'student' (or 'all') from the
...
// Student pages often open their own $conn (via `new mysqli(...)`)
// without ever loading Teacher/Database.php, so checking isset($conn)
// alone isn't enough — check whether the function itself is already
// defined instead, and always load Database.php if not.
if (!function_exists('getNotificationsForRole')) {
    require_once __DIR__ . '/../Teacher/Database.php';
}

$notifItems = getNotificationsForRole($conn, 'student');
?>
